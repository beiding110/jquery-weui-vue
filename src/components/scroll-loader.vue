...
export default {
    ...
    computed: {
        ...
        currentPgPath() {
            return this.isVRout ? this.$route.fullPath : window.location.href;
        }
    },
    methods:{
        ...
        storageScroll(e) {
            ...
            obj['$scrollCache:' + this.currentPgPath] = e.target.scrollTop;
            if(this.isVRout) {
                this.setSession(obj);
            } else {
                setSession(obj);
        ...
        setScroll() {
            var cache = this.getSession('$scrollCache:' + this.currentPgPath);
            this.$refs['scroll-con'].scrollTop = cache || 0;
        },
        clearStorageScroll() {
            var obj = {};
            obj['$scrollCache:' + this.currentPgPath] = 0;
            if(this.isVRout) {
                this.setSession(obj);
            } else {
                setSession(obj);
